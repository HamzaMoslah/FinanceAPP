...
public class IndicesActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        TableRow tableRow = (TableRow) getLayoutInflater().inflate(R.layout.table_indices, null);
        TextView tv;

        //Filling in cells
        tv = (TextView) tableRow.findViewById(R.id.nom);
        tv.setText("Nom");
        tv = (TextView) tableRow.findViewById(R.id.ouv);
        tv.setText("Ouverture");
    ...
    private void loadJSON() {
        ...
        Call<JSONResponseIndex> call = request.getJSONIndex();
        call.enqueue(new Callback<JSONResponseIndex>() {
            @Override
            public void onResponse(Call<JSONResponseIndex> call, Response<JSONResponseIndex> response) {
                JSONResponseIndex jsonResponse = response.body();
                indices.addAll(Arrays.asList(jsonResponse.getIndices()));
                showList();
                mProgressDialog.dismiss();
            }

            @Override
            public void onFailure(Call<JSONResponseIndex> call, Throwable t) {
                Log.d("Error", "");
                mProgressDialog.dismiss();
            }
        });
    ...
    private void showList() {
        for (Indices indice : indices) {
            TableRow tableRow1 = (TableRow) getLayoutInflater().inflate(R.layout.table_indices, null);
            TextView tv1;
            //Filling in cells
            tv1 = (TextView) tableRow1.findViewById(R.id.nom);
            tv1.setText(indice.getNom());
            tv1 = (TextView) tableRow1.findViewById(R.id.ouv);
            tv1.setText(indice.getOuverture());
